...
**Solution:** 
- **The system now automatically filters invalid records** - you should only see this if preprocessing changed
...
### Issue 3: "Low schema coverage" or "All records skipped"

**Problem:** Most/all S3 records are being filtered out.

**Solution:**
- Run `validate_s3_data.py` to see detailed skip reasons
- **Common causes:**
  - Reference data wasn't generated by the endpoint (missing preprocessing)
  - metadata field is empty or malformed
  - decisionResult is missing from metadata
  - Schema mismatch with current pipeline version
- Regenerate reference data using the current endpoint version
